...
\*******************************************************************/
	...
	if (isset($_GET['key']) && is_string($_GET['key']) && preg_match('/^[a-z\d]{32}$/i', $_GET['key']))
	{
		...
		// activate user
		$check_result = smart_mysql_query("SELECT status FROM cashbackengine_users WHERE activation_key='$activation_key' LIMIT 1");
        if (mysqli_num_rows($check_result) > 0)
		{
			...
			{
				smart_mysql_query("UPDATE cashbackengine_users SET status='active', activation_key='' WHERE activation_key='$activation_key' AND login_count='0' LIMIT 1");
