...
\*******************************************************************/
	...
	if (isset($_POST["action"]) && $_POST["action"] == "import")
	{
		...
		else
		{
			...
			$aff_row = mysqli_fetch_array($aff_result);
		...
		if (count($errors) == 0)
		{
			...
			foreach ($alldata as $key => $row)
			{
				...
				$program_id_e		= mysqli_real_escape_string($conn, $new_row[$program_id]);
				$subid_e			= mysqli_real_escape_string($conn, $new_row[$sub_id]);
				...
				if (!empty($cashbackengine_status))
				{
					...
					$cashback_row = mysqli_fetch_array($cashback_result);

					$cashback_store = mysqli_real_escape_string($conn, $cashback_row['title']);
			...
			<div style="width:80%;" class="success_box">Your CSV has been successfully uploaded</div>
			 ...
			 <tr>
				<td style="padding: 10px;" nowrap="nowrap" align="left" valign="middle">
					...
					Cashback: <b><?php echo $_SESSION['total_cashback']; ?></b>
				</td>
			</tr>
			</table>
			<br/>
