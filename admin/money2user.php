...
\*******************************************************************/
	...
	if (isset($_POST["action"]) && $_POST["action"] == "addmoney")
	{
			...
			$transaction_id	= mysqli_real_escape_string($conn, getPostParameter('transaction_id'));
		...
		if (count($errors) == 0)
		{
				...
				if ($retailer_id > 0)
				{
					...
					if (mysqli_num_rows($sresult) > 0)
					{
						...
						$retailer	= mysqli_real_escape_string($conn, $srow['title']);
						$program_id = mysqli_real_escape_string($conn, $srow['program_id']);
