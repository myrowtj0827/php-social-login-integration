	...
	if (isset($_POST["action"]) && $_POST["action"] == "import")
	{
		...
		if (count($errors) == 0)
		{
			...
			foreach ($csv->data as $key => $row)
			{
				...
				$title_e			= mysqli_real_escape_string($conn, $new_row[$title_id]);
				$network_e			= mysqli_real_escape_string($conn, $new_row[$network_id]);
				$program_e			= $new_row[$program_id];
				$category_e			= mysqli_real_escape_string($conn, $new_row[$category_id]);
				$image_e			= $new_row[$image_id];
				$url_e				= $new_row[$url_id];
				$cashback_e			= $new_row[$cashback_id];
				$cashback_sign_e	= $new_row[$cashback_sign_id];
				$description_e		= mysqli_real_escape_string($conn, $new_row[$description_id]);
				$conditions_e		= mysqli_real_escape_string($conn, $new_row[$conditions_id]);
				...
				if ($title_e && $network_e && $image_e && $url_e && $cashback_e && $cashback_sign_e)
				{
					...
					else if (is_string($network_e))
					{
						...
						if (mysqli_num_rows($check_network) != 0)
						{
							$network_row = mysqli_fetch_array($check_network);
						...
						if ($category_e != "")
						{
							...
							else if (is_string($category_e))
							{
								...
								foreach ($categories_list as $kk=>$category_name)
								{
									...
									{
										$category_row = mysqli_fetch_array($check_category);
