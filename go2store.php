...
\*******************************************************************/
	...
	if (isset($_GET['id']) && is_numeric($_GET['id']))
	{
		...
		if (mysqli_num_rows($result) > 0)
		{
			...
			$cid_query = "SELECT MAX(click_id) AS click_id FROM cashbackengine_clickhistory";
            $new_click_res = smart_mysql_query($cid_query);
            $crow = mysqli_fetch_array($new_click_res);
            $click_id = '&click='.$crow['click_id'];

			if ($goto != "")
			{
				// redirect user
				header("Location: ".$goto.$click_id);
				exit();
			}
		}
		else
