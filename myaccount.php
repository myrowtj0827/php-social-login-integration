...
?>
	...
	?>
		...
		<?php while ($row_featured = mysqli_fetch_array($result_featured)) { $cc++; ?>
			<div class="col-xs-6 col-sm-3 col-md-3"><div class="imagebox"><a href="<?php echo GetRetailerLink($row_featured['retailer_id'], $row_featured['title']); ?>"><img src="<?php if (!stristr($row_featured['image'], 'http')) echo SITE_URL."img/"; echo $row_featured['image']; ?>" width="<?php echo IMAGE_WIDTH; ?>" height="<?php echo IMAGE_HEIGHT; ?>" alt="<?php echo $row_featured['title']; ?>" title="<?php echo $row_featured['title']; ?>" border="0" /></a></div></div>
		<?php } ?>
		</div>
		<div style="clear: both"></div>
	<?php } // end featured store ?>
