...
\*******************************************************************/
	...
	function getRepliesNum($message_id)
	{
		...
		$row = mysqli_fetch_array(smart_mysql_query("SELECT COUNT(answer_id) as total_replies FROM cashbackengine_messages_answers WHERE message_id='".(int)$message_id."' AND user_id='".(int)$userid."' AND is_admin='1'"));		
...
?>
	...
	<?php if (isset($_GET['msg']) and $_GET['msg'] == 1) { ?>
		<div class="alert alert-success"><?php echo CBE1_MYMESSAGES_SENT; ?></div>
	<?php }else if (!(isset($_REQUEST['mid']) && is_numeric($_REQUEST['mid']))) { ?>

		<h1><?php echo CBE1_MYMESSAGES_TITLE; ?></h1>

		<p align="center" class="hidden-xs"><img src="<?php echo SITE_URL; ?>images/icon_support.png" border="0" /></p>
		...
		<div class="row">
			...
			<table align="center" width="100%" border="0" cellpadding="3" cellspacing="0">
	          <tr>
	            ...
	            <td align="left" valign="top"><input type="text" class="form-control" name="subject" value="<?php echo getPostParameter('subject'); ?>" placeholder="<?php echo CBE1_MYMESSAGES_SUBJECT; ?>" size="53" /></td>
	          </tr>
	          <tr>
	            <td align="right" valign="top">&nbsp;</td>
	            <td align="left" valign="top"><textarea cols="50" rows="10" class="form-control" placeholder="<?php echo CBE1_MYMESSAGES_MESSAGE; ?>" name="message"><?php echo getPostParameter('message'); ?></textarea></td>
	          </tr>
	          <tr>
	            <td align="right" valign="middle">&nbsp;</td>
	...
	if (isset($_REQUEST['mid']) && is_numeric($_REQUEST['mid']))
	{
		...
		<?php
			...
			if (mysqli_num_rows($aresult) > 0)
			{
				...
				while ($arow = mysqli_fetch_array($aresult)) { $cc++;
		?>
		...
		<table align="center" border="0" cellpadding="3" cellspacing="0">
          <tr>
            <td align="left" valign="top">
				...
				<?php } ?>
				<textarea cols="52" rows="5" class="form-control" name="answer" required="required"><?php echo getPostParameter('answer'); ?></textarea>
			</td>
          </tr>
          <tr>
			<td align="left" valign="middle">
	...
	<?php if ($mtotal > 0) { ?>
        ...
        <?php }else{ ?>
				<div class="alert alert-info"><?php echo CBE1_MYMESSAGES_NO; ?></div>
        <?php } ?>
