...
\*******************************************************************/
	...
	if (isset($_POST['withdraw']) && $_POST['withdraw'] != "")
	{
		...
		else
		{
			...
			$presult = smart_mysql_query("SELECT * FROM cashbackengine_pmethods WHERE pmethod_id='$payment_method' AND status='active' LIMIT 1");
		...
		if (count($errs) == 0)
		{
			...
			$rp_query = "INSERT INTO cashbackengine_transactions SET reference_id='$reference_id', user_id='$userid', payment_type='Withdrawal', payment_method='$payment_method', payment_details='$payment_details', transaction_commision='$transaction_commission', amount='$amount', status='request', created=NOW()";
...
?>
	...
	<?php }else{ ?>
		...
		<?php if (GetUserBalance($userid, 1) >= MIN_PAYOUT) { ?>
			...
			<?php if (isset($_SESSION['password_verified']) && $_SESSION['password_verified'] > time()) { ?>
				...
				<?php 
						...
						$pquery = "SELECT * FROM cashbackengine_pmethods WHERE pmethod_id='$payment_method_id' AND status='active' LIMIT 1";
				...
				?>
					...
					<tr>
						...
						<td align="left" valign="middle"><b><?php echo DisplayCashback($prow['commission']); ?></b></td>
					</tr>
					<?php } ?>
				<tr>
		...
		<?php }else{ ?>
			<div class="alert alert-info"><?php echo CBE1_WITHDRAW_MSG3; ?> <b><?php echo DisplayMoney(MIN_PAYOUT); ?></b> <?php echo CBE1_WITHDRAW_MSG4; ?></div>
		<?php } ?>
